	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) 
		...
		try{
			...
			//글추가 요청이 들어 왔을때..
				...
				//upload()메소드를 호출해..글쓰기 화면에서 입력한 글관련 정보를
				//HashMap에  key와 value을 쌍으로 저장 합니다.
				//그런후...
				//글 입력시 추가적으로 업로드할 파일을 선택하여 글쓰기 요청을 했다면..
				//업로드할 파일명, 입력한 글제목, 입력한 글내용을 key/value형태의 값들로 저장되어 있는
				//HashMap을 리턴 받는다.
				//그렇지 않은 경우에는?????????????????
				//업로드한 파일명을 제외한 입력한 글제목, 입력한 글내용을 key/value형태의 값들로 저장되어 있는
				//HashMap을 리턴 받는다.
				Map<String, String> articleMap = upload(request, response);	
				
				//HashMap에 저장된 작성한  글데이터(업로드할 파일명, 입력한 글제목, 입력한 글내용)를 
				//HashMap에서 꺼내오기
				...
				//DB에 새글정보를 추가하기 위해 사용자가 입력한 글정보 + 업로드할 파일명을
				//ArticleVO객체의 각변수에 저장
				...
				//요약 : 글쓰기 창에서 입력된 정보를 HashMap에서 꺼내어서 articleVO에  설정 한후
				//      BoardService객체의 addArticle()메소드로 전달하여 INSERT작업을 명령 함.
				boardService.addArticle(articleVO);
				
				//DB에 새글을 추가하고  BoardController서블릿에서
				// /board02/listArticle.jsp로 이동하여
				// 전체 글정보를 다시 DB에서 검색하여 보여 주기위해  다음과 같은 요청 주소를  저장
