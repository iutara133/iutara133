...
//@WebServlet("/board/*") // /board/articleForm.do 
						//  /board/viewArticle.do?articleNO=글번호
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) 
		...
		try{
			...
			//글을 쓸수 있는 화면으로 이동 시켜줘~ 라는 요청이 들어 왔을떄..
				...
				nextPage = "/board03/articleForm.jsp";//글쓰기 화면
			...
			}else if(action.equals("/viewArticle.do")){//글제목을 클릭 했을떄..
													   //상세볼 글정보를 검색 해줘~				
				//요청받은값
				//상세볼 글에 대한 글번호를 request에서 꺼내오기
				String articleNO  = request.getParameter("articleNO");
				
				//서비스 역할을 하는 객체의 메소드 호출 하여 검색 요청을 함.
				//반환값 -> 검색한 글정보가 저장된 ArticleVO객체
				articleVO = boardService.viewArticle(Integer.parseInt(articleNO));
				
				//검색한 글정보가 저장되어 있는 ArticleVO객체의 정보를 View페이지로 전달 하기 위해
				//request내장객체 영역에 저장하여 유지 시키기 
				request.setAttribute("article", articleVO);
				
				//재요청할 뷰페이지 주소를 nextPage변수에 저장
